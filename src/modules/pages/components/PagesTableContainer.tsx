...
import { editIcon, eyeIcon } from '../../../config/variables';
import { navigateRoute } from '../../../utils/HelperFunctions';
import DeleteButton from '../../../components/buttons/DeleteButton';
import TableSkeleton from '../../../components/loaders/TableSkeleton';
import { ITableContainerProps } from '../../../types/Interfaces';
import { IPageTableItem } from '../types/interfaces';
import { useTranslation } from 'react-i18next';

const PagesTableContainer = ({ tableHeaders, data, loading, noDataMessage, lessColumns }: ITableContainerProps<IPageTableItem[]>) => {
          ...
          <>
            {data?.map((item: any, index: number) => (
              <div className="item has_logo fuel_types" key={index}>
                ...
                {/* {hasPermission(['fuel-types.edit']) && ( */}
                <div className="column actions actions_col">
                  <div
                    className="action_btn edit"
                    onClick={(e) => {
                      navigateRoute(e, `/pages/page-sections/${item.id}`, navigate);
                    }}
                  >
                    {eyeIcon}
                  </div>
                  <div
                    className="action_btn edit"
                    onClick={(e) => {
                      navigateRoute(e, `/pages/create-page/${item.id}`, navigate);
                    }}
                  >
                    {editIcon}
                  </div>
                  <DeleteButton deleteRoute={`/cms/admin/pages`} queryKey={`pages`} id={item.id} />
                </div>
                {/* )} */}
              </div>
            ))}
